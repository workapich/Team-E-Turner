...
if (isset($_GET['broj']) && !empty($_GET['broj'])) {
    ...
    if (isset($search_arrDva) && !empty($search_arrDva)) {
        ...
        $query= "INSERT INTO `narudzbine`(`id_user`, `email`,`narudzbina`, `date_time`) VALUES  ('{$_SESSION['id_user']}', '{$_SESSION['username']}', '$whereDva','$today')";
...
if (isset($sql_str)) {
    $sql = "SELECT id_omiljeno ,vozila.id_vozila, vozila.slika ,vozila.cena , vozila.model from omiljeno INNER JOIN vozila ON omiljeno.id_vozila = vozila.id_vozila where id_user = {$_SESSION['id_user']}";
...
window.onclick = function(event) {
  if (!event.target.matches('.dropbtn')) {
    ...
    <header>
<img src="VladVorkPNG.png" alt="logo" class="logo" width="100px">
                        ...
                        foreach ($products as $key => $value) {
                            ...
                            echo '<div class="col" data-item-id='.$id.'>
                               ...
                               echo ' <p>'.$value['model'].'<p>
                                ...
                                if (isset($_SESSION['username']) OR isset($_SESSION['id_user'])) 
            {
                                echo '<a href="favouriteWeb.php?broj='.$id.'" class="cta"><button>Buy</button></a>';
            }
            else
            {
